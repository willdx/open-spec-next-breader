...
function CustomPopup() {
  ...
  const handleExtractWebClick = async () => {
    if (isSaving) return
    ...
    try {
      console.log("📖 正在提取当前标签页内容...")
      // 抓取当前网页内容
      const extractedContent = await webContentExtractor.extractFromCurrentTab()

      if (!extractedContent) {
        throw new Error("无法提取网页内容")
      }

      console.log("✅ 网页内容提取成功，准备保存...")
      console.log("📄 文档信息:", {
    ...
    <div className="w-80 bg-white rounded-lg shadow-lg p-4">
      ...
      <div className="space-y-3">
        <button
          ...
          disabled={isSaving}
          className={`w-full rounded-lg px-4 py-3 font-medium transition-colors flex items-center justify-center ${
            isSaving
