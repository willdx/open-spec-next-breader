...
export class DocumentStorageService {
  ...
  async getDocuments(): Promise<DocumentData[]> {
    const documents = await this.storage.get<DocumentData[]>(
  ...
  async getLastReadDocument(): Promise<DocumentData | null> {
    const documents = await this.getDocuments()
    const documentsWithReadTime = documents.filter(
      (doc) => doc.lastReadTime !== undefined
    )
    ...
    const sortedDocuments = documentsWithReadTime.sort(
